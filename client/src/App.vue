...
<template>
  <header class="flex gap-2 justify-center flex-wrap">
    <RouterLink class="text-center w-fit p-4" active-class="underline" to="/"
      >All Photos</RouterLink
    >
    <RouterLink
      class="text-center w-fit p-4"
    ...
    <template v-if="permissionStore.adminView">
      <RouterLink
        ...
        active-class="underline"
        to="/auth"
        >{{ icon }} Login & Credentials</RouterLink
      >
      <RouterLink
        class="text-center w-fit p-4"
        active-class="underline"
        to="/admin/upload"
        >{{ icon }} Upload</RouterLink
      >
      <RouterLink
        class="text-center w-fit p-4"
        active-class="underline"
        to="/auth-test"
        >{{ icon }} Passkey Testing</RouterLink
      >
    </template>
    <a
      class="text-center w-fit p-4 md:ml-auto"
      href="https://github.com/yifeiyin/captured"
      target="_blank"
      >Site Source <span class="inline-block -scale-x-100">⎋</span></a
    >
    <button
      class="text-center w-fit p-4"
      title="Authenticate"
