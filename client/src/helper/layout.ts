...
export type PhotoGroupingStrategy = <P>(
  ...
  photos: P[],
  getPhotoType: (p: P) => PhotoOrientation
) => { items: P[]; type: PhotoOrientation }[];

export const strictlyOrdered: PhotoGroupingStrategy = (
  config,
  photos,
  getPhotoType
) => {
  ...
  photos.forEach((p) => {
    if (
      currentGroup.items.length >= config[currentGroup.type] ||
      currentGroup.type !== getPhotoType(p)
    ) {
 ...
 * @param getPhotoType
 */
export const looselyOrdered: PhotoGroupingStrategy = (
  config,
  photos,
  getPhotoType
) => {
  ...
  if (verticalLeftOvers.length > 0 || horizontalLeftOvers.length > 0) {
    grouped.push({
      items: [...verticalLeftOvers, ...horizontalLeftOvers],
      type: '|',
    });
  }
