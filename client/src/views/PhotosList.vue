...
trpc.photos.list
  .query({
    collectionName: props.collection === 'none' ? null : props.collection,
    tagName: props.tag,
  })
  .then((res) => {
    photos.value = res;
...
async function onSizeChange(zoomIn: number) {
  ...
  currentSize.value = Math.max(
    0,
    Math.min(AllowedSizes.length - 1, currentSize.value - zoomIn)
  );
}
...
const groupedPhotos = computed(() => {
  return looselyOrdered(config.value, photos.value, (x) =>
    x.width > x.height ? '-' : '|'
  );
});
...
function imageLinkWithContext(photo: Photo) {
  return props.collection
    ? `/collections/${props.collection}/photos/${photo.id}`
    : props.tag
      ? `/tags/${props.tag}/photos/${photo.id}`
      : `/photos/${photo.id}`;
}
</script>
...
<template>
  <div class="flex justify-between">
    <h1 v-if="props.collection" class="text-2xl font-bold p-4">
      Collection:
      <span v-view-transition-name="'collection' + props.collection">{{
        props.collection
      }}</span>
    </h1>
    <h1 v-else-if="props.tag" class="text-2xl font-bold p-4">
      Tag:
      <span v-view-transition-name="'tag' + props.tag">{{ props.tag }}</span>
    </h1>
    ...
    <div class="field has-addons px-4">
      <p class="control">
        <button
          class="button"
          :disabled="currentSize === AllowedSizes.length - 1"
          @click="onSizeChange(-1)"
        >
          –
        </button>
      </p>
      <p class="control">
        <button
          ...
          @click="onSizeChange(currentSize - 3)"
        >
          ⌾
        </button>
      </p>
  ...
  <div class="flex flex-col gap-0.5">
    <div
      ...
      class="flex flex-row items-center justify-center gap-0.5"
    >
      <div
        v-for="item of group.items"
        :key="item.id"
        :style="{ width: `${100 / config[group.type]}%` }"
      >
        <RouterLink :to="imageLinkWithContext(item)">
          <CapturedImage
            :key="item.id"
            :image="item"
            :size="100 / config[group.type]"
          />
        </RouterLink>
      </div>
    </div>
