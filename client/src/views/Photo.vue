...
const backRoute = computed(() => {
  ...
  return props.collection ? `/collections/${props.collection}` : `/tags/${props.tag}`;
});
...
const photosList = useStorage<Photo[]>(`photos-${JSON.stringify(props)}`, [], undefined, { serializer: StorageSerializers.object });
const prevRoute = computed(() => {
  ...
  return props.collection ? `/collections/${props.collection}/photos/${previous}` :
    props.tag ? `/tags/${props.tag}/photos/${previous}` :
      `/photos/${previous}`;
});
const nextRoute = computed(() => {
  ...
  if (!next) return null;
  return props.collection ? `/collections/${props.collection}/photos/${next}` :
    props.tag ? `/tags/${props.tag}/photos/${next}` :
      `/photos/${next}`;
});
