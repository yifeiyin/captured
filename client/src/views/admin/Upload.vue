...
const { isOverDropZone } = useDropZone(dropZoneRef, {
  onDrop: (files) => { onSelectFiles(files ?? []) },
...
<template>
  <div class="grid place-items-center grid-cols-2 p-20">
    <div ref="dropZoneRef" class="relative h-40 col-span-full place-self-stretch border-2 rounded-md border-dashed bg-gray-100 flex items-center justify-center cursor-pointer" :style="{ backgroundColor: isOverDropZone ? 'green' : '' }" @click="fileSelectRef?.click()">
      <div v-if="selectedFiles.length">{{ selectedFiles.length }} file(s) selected.</div>
      <div v-else>Drag file to here</div>
      <input ref="fileSelectRef" type="file" class="hidden" multiple @change="onSelectToUpload($event)">
    </div>
    <div class="col-span-full">
      <ul>
        <li v-for="(f, index) in selectedFiles" :key="index" class="border rounded-sm m-2 text-center">
          <img :id="'pending-upload-file' + index" :src="generateURL(f)" :alt="f.metadata.filename" />
          <span>{{ f.metadata.type }}, {{ f.metadata.filename }}</span>
          <button class="button is-small" @click="removeUpload(index)">Remove</button>
        </li>
      </ul>
    </div>
