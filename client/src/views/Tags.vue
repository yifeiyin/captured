...
const tags = useStorage<Awaited<ReturnType<typeof trpc.tags.all.query>>>(
  `tags`,
  []
);
  ...
  <div class="flex-1 flex flex-col gap-1 justify-center items-center">
    <div class="flex flex-row flex-wrap gap-1 justify-center items-center">
      <template v-if="tags">
        <template v-for="item of tags" :key="item.id">
          ...
          >
            <span
              v-view-transition-name="'tag' + item.name"
              class="font-bold"
              >{{ item.name }}</span
            >
            <span class="">({{ item.photosCount }})</span>
          </RouterLink>
    ...
    <div v-if="permissionStore.adminView">
      <input
        ...
        type="text"
        class="input is-link"
        placeholder="Rename or create new tag"
        @keyup.enter="create"
      />
    </div>
  </div>
