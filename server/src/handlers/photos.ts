...
export const photos = router({
      ...
      if (collectionName === null) {
        ...
        const collectionId = (
          await db().query.collections.findFirst({
            where: eq(t.collections.name, collectionName),
          })
        )?.id;
        ...
        const tagId = (
          await db().query.tags.findFirst({ where: eq(t.tags.name, tagName) })
        )?.id;
        if (!tagId) return [];
        const photoList = (
          await db().query.photoTags.findMany({
            where: eq(t.photoTags.tagId, tagId),
          })
        ).map((x) => x.photoId);
  ...
  get: P.public.input(z.number()).query(async (opts) => {
    const entry = await db().query.photos.findFirst({
      where: eq(t.photos.id, opts.input),
      with: { tags: true, resources: true, collection: true },
    });
    if (!entry) {
    ...
    .mutation(async (opts) => {
      await db()
        ...
        .set({
          ...
          collectionId: opts.input.collection,
        })
        .where(eq(t.photos.id, opts.input.id));
      return { message: 'OK' };
    }),
  ...
  getTags: P.public.input(z.number()).query(async (opts) => {
    ...
    const photoTags = await db().query.photoTags.findMany({
      where: eq(t.photoTags.photoId, photoId),
    });
    ...
    .mutation(async (opts) => {
      ...
      if (selected) {
        ...
        await db()
          .delete(t.photoTags)
          .where(
            and(eq(t.photoTags.photoId, photoId), eq(t.photoTags.tagId, tagId))
          );
      }
      return { message: 'OK' };
    }),
  ...
  upload: P.auth
    ...
    .mutation(async (opts) => {
      await Promise.all(
        opts.input.map(async (img) => {
          const rawBytes = Buffer.from(img.bytes, 'base64');
          const photo = (
            await db().insert(t.photos).values({ description: '' }).returning()
          )[0];
