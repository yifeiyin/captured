...
const config = () => {
  ...
  if (!secret) {
    throw new Error(
      'Secret is empty. For local development, make sure to create .dev.vars file'
    );
  }
  return {
...
export const authTest = router({
  ...
  registrationVerify: P.auth
    ...
    .mutation(async (opts) => {
      const verifiedJwt = await jwtVerify(opts.input.jwt, config().jwt.secret, {
        algorithms: [config().jwt.alg],
        audience: 'testing.regVerify',
      });
      const payload = z
        .object({ userName: z.string(), challenge: z.string() })
        .parse(verifiedJwt.payload);
  ...
  authenticationOptions: P.public
    .input(
      z.object({
        userName: z.string(),
        userVerification: z.enum(['discouraged', 'required', 'preferred']),
    ...
    .query(async (opts) => {
      const passKeyStored = await db()
        .select()
        .from(t.passkeys)
        .where(eq(t.passkeys.userName, opts.input.userName));
...
export const auth = router({
  currentSignInOption: P.public.query(async () => {
  ...
  registrationOptions: P.auth.query(async () => {
  ...
  authenticationOptions: P.public.query(async (_opts) => {
    const passKeyStored = await db().query.passkeys.findMany();
    ...
    .mutation(async (opts) => {
      ...
      const passKeyStored = (
        await db()
          .select()
          .from(t.passkeys)
          .where(eq(t.passkeys.keyId, opts.input.response.id))
      )[0];
  ...
  authenticateWithToken: P.public.input(z.string()).mutation(async (opts) => {
    if (!opts.input) {
      return { token: null };
    }
    ...
    if (
      passKeyStored.length === 0 &&
      opts.input === new TextDecoder().decode(config().jwt.secret)
    ) {
