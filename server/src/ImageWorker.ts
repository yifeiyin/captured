...
export async function saveImage(imageId: number, type: string, blob: Blob, width: number, height: number, size: number, isOriginal: boolean) {
  let bucket: R2Bucket = cfEnv.getStore()!.CF_R2_BUCKET;
  const uniqueId = `prod/${imageId}/${Date.now()}.${Math.random().toString(36).substring(2).slice(0, 10)}`; // config1

  await bucket.put(uniqueId, blob, { httpMetadata: { contentType: type, cacheControl: 'public, max-age=604800, immutable' } });

  await db().insert(t.photoResources).values({ photoId: imageId, fileId: uniqueId, type, width, height, size, isOriginal });

  if (isOriginal) {
    await db().update(t.photos).set({ width, height }).where(eq(t.photos.id, imageId));
  }
}


async function saveImageWrapper(imageId: number, fmt: string, buf: Buffer, w: number, h: number, original: boolean = false) {
  await saveImage(imageId, `image/${fmt}`, new Blob([buf]), w, h, buf.byteLength, original);
}
...
function extractBMPImageData(bmpData: Uint8ClampedArray): Uint8ClampedArray {
  ...
  let height = bmpData[22] | (bmpData[23] << 8) | (bmpData[24] << 16) | (bmpData[25] << 24);
  ...
  if (bytesPerPixel !== 3 && bytesPerPixel !== 4) {
    throw new Error("Unsupported BMP format: Only 24-bit and 32-bit BMPs are supported");
  }
...
export async function processImage(data: ImageWorkerMessage) {
  ...
  const ab = addAlphaChannel(extractBMPImageData(new Uint8ClampedArray(await thumbnail.getBuffer(JimpMime.bmp)))) // .slice(0, thumbnail.width * thumbnail.height);

  const blurhash = encode(ab, thumbnail.width, thumbnail.height, 4, 3);

  await db().update(t.photos).set({ blurhash }).where(eq(t.photos.id, data.photoId));

  const original = await Jimp.fromBuffer(data.rawBytes);
  await saveImageWrapper(data.photoId, 'jpg', await original.getBuffer("image/jpeg"), original.bitmap.width, original.bitmap.height, true);
  ...
  for (let i = 2; i < 8; i++) {
    ...
    a.resize({ w: size });
    await saveImageWrapper(data.photoId, 'jpg', await a.getBuffer(JimpMime.jpeg), a.width, a.height);
  }
}
